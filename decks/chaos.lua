...
local function chaos_tarot_target(def, card)
    ...
    if card and card.ability and card.ability.chaos_tarot_suit then
        return card.ability.chaos_tarot_suit
    end
    return card and card.chaos_tarot_suit or def.vanilla
end
    ...
    for _, short_key in ipairs({ 'sun', 'star', 'moon', 'world' }) do
        SMODS.Consumable:take_ownership(short_key, {
            ...
            set_sprites = function(self, card, front)
                card.chaos_tarot_visual = chaos_tarot_apply_sprite(card)
                card.chaos_tarot_active = chaos_tarots_enabled()
            ...
            update = function(self, card, dt)
                ...
                if card.chaos_tarot_visual ~= visual then
...
local function chaos_preview_copyable_card(card)
...
local function chaos_preview_card_lookup()
    local lookup = {}
    local seen = {}
    local function add(card)
        if not chaos_preview_copyable_card(card) or seen[card] then return end
        seen[card] = true
...
local function chaos_resolve_preview_card(value, lookup)
    if chaos_preview_copyable_card(value) then return value, false end
...
local function append_chaos_card_rows(deck_tables, suit_cards, unplayed_only, groups)
    ...
    local card_lookup = chaos_preview_card_lookup()
        ...
        for _, suit_key in ipairs(group) do
            ...
            if type(source) == 'table' then
                for _, value in ipairs(source) do
                    local card, greyed = chaos_resolve_preview_card(value, card_lookup)
